...
function Header() {
    ...
    return <header className="site-header professional-header"><div className="header-main"><Link to="/" className="brand"><span className="brand-mark">A</span><span>andré<span className="brand-shop">shop</span></span></Link><div className="header-location"><span className="location-icon">⌖</span><div><small>Livrer à</small><strong>Douala, Cameroun</strong></div></div><form className="header-search" onSubmit={submit}><span>⌕</span><input aria-label="Rechercher" placeholder="Que recherchez-vous aujourd'hui ?" value={search} onChange={(e) => setSearch(e.target.value)} /><button type="submit">Rechercher</button></form><div className="header-actions">{user ? <Link to="/compte" className="header-account"><span className="account-icon">♙</span><span><small>Bonjour</small><strong>{user.name?.split(' ')[0] || 'vous'}</strong></span></Link> : <button className="header-account" onClick={() => setAuthOpen(true)}><span className="account-icon">♙</span><span><small>Mon espace</small><strong>Se connecter</strong></span></button>}{user?.role === 'admin' && <Link to="/admin" className="header-dashboard" aria-label="Ouvrir le tableau de bord"><span className="dashboard-icon">▦</span><span><small>Administration</small><strong>Dashboard</strong></span></Link>}<Link to="/checkout" className="header-cart"><span className="cart-icon">♧</span><span><small>Votre panier</small><strong>Panier <b><CartCount /></b></strong></span></Link>{user && <button className="header-logout" onClick={logout} aria-label="Se déconnecter">↪</button>}</div></div><div className="header-nav-wrap"><div className="header-nav-inner"><button className={'mobile-nav-trigger ' + (mobileNavOpen ? 'active' : '')} onClick={() => { setMobileNavOpen(!mobileNavOpen); setMenuOpen(false); }} aria-expanded={mobileNavOpen}><span>☰</span> Menu</button><button className={'category-menu-trigger ' + (menuOpen ? 'active' : '')} onClick={() => { setMenuOpen(!menuOpen); setMobileNavOpen(false); }}><span>▦</span> Toutes les catégories <i>⌄</i></button><nav className="main-nav"><Link to="/catalogue">Nos produits</Link><a href="/#offers">Promotions</a><a href="/#services">Nos services</a><a href="/#engagement">Pourquoi André Shop ?</a><Link to="/contact">Nous contacter</Link><Link to="/compte">Mon espace</Link></nav><div className="nav-highlight">✓ Paiement sécurisé par Nokash</div></div>{mobileNavOpen && <nav className="mobile-nav-panel"><Link to="/catalogue" onClick={() => setMobileNavOpen(false)}>Nos produits</Link><a href="/#offers" onClick={() => setMobileNavOpen(false)}>Promotions</a><a href="/#services" onClick={() => setMobileNavOpen(false)}>Nos services</a><a href="/#engagement" onClick={() => setMobileNavOpen(false)}>Pourquoi André Shop ?</a><Link to="/contact" onClick={() => setMobileNavOpen(false)}>Nous contacter</Link><Link to="/compte" onClick={() => setMobileNavOpen(false)}>Mon espace</Link></nav>}{menuOpen && <div className="mega-menu"><div className="mega-menu-inner"><div className="mega-intro"><p className="eyebrow">EXPLORER LA BOUTIQUE</p><h3>Tout pour équiper<br /><em>vos projets.</em></h3><Link to="/catalogue" onClick={() => setMenuOpen(false)} className="text-link">Voir le catalogue <span>↗</span></Link></div><div className="mega-categories">{popular.map((category, index) => { const image = category.image_url || categoryImages[category.slug] || fallbackImages[index % fallbackImages.length]; return <Link key={category.id || category.slug} to={'/catalogue?category=' + (category.slug || category.id)} onClick={() => setMenuOpen(false)}><span className="mega-image"><img src={image} alt="" loading="lazy" /></span><span className={'mega-icon mega-icon-' + ((index % 4) + 1)}>{['⌂','◈','▣','✦'][index % 4]}</span><span><strong>{category.name}</strong><small>Découvrir la sélection</small></span><b>↗</b></Link>; })}</div></div></div>}</div></header>;
}
...
function CheckoutPage() {
    ...
    const placeOrder = async (e) => {
        ...
        setError('');
        try {
            const result = await request('/customer/orders', { method:'POST', token, body:{ shipping_address:{ full_name:values.name, phone:values.phone, line1:values.address, city:values.city, country_code:'CM' } } });
            const order = result.data || result;
            const number = order.number || order.order_number;
            const phone = '237' + values.phone.replace(/\D/g, '').replace(/^237/, '');
            await request('/customer/orders/' + number + '/payments', { method:'POST', token, body:{ payment_method:values.paymentMethod, user_phone:phone, idempotency_key:'web-' + number + '-' + values.paymentMethod } });
            await refreshCart();
            notify('Commande créée. Confirmez le paiement sur votre téléphone.');
...
function AdminPage() {
    ...
    const load = async (section = tab, pageNumber = 1) => {
        ...
        try {
            if (section === 'overview') {
                setStats(unwrap(await request('/admin/dashboard', { token })));
                setData([]);
                setPagination(null);
            } else {
...
function AdminOverview({ stats, money }) {
    const orders = stats?.recent_orders || [], lowStock = stats?.low_stock_items || [];
    return <div className="admin-overview"><div className="admin-welcome"><div><p className="eyebrow">RÉSUMÉ OPÉRATIONNEL</p><h2>Une vue claire pour décider vite.</h2><p>Suivez votre activité commerciale et les opérations prioritaires depuis un seul espace.</p></div><div className="admin-welcome-shape">A</div></div><div className="admin-kpis"><div><span>Produits actifs</span><strong>{stats?.active_products || 0}</strong><small>Références au catalogue</small></div><div><span>Commandes</span><strong>{stats?.orders || 0}</strong><small>Toutes périodes</small></div><div><span>Paiements confirmés</span><strong>{money(stats?.confirmed_payments_total || 0)}</strong><small>Transactions réussies</small></div><div className={stats?.low_stock_count ? 'warning' : ''}><span>Alertes stock</span><strong>{stats?.low_stock_count || 0}</strong><small>{stats?.low_stock_count ? 'À réapprovisionner' : 'Tout est sous contrôle'}</small></div></div><div className="admin-panels"><div className="admin-panel"><div className="panel-heading"><div><p className="eyebrow">ACTIVITÉ RÉCENTE</p><h3>Dernières commandes</h3></div><span>↗</span></div>{orders.map((order) => <div className="admin-list-row" key={order.id}><div><strong>{order.number}</strong><small>{order.customer_name || 'Client'}</small></div><span className={`status status-${order.status}`}>{formatStatus(order.status)}</span><b>{money(order.total)}</b></div>)}{!orders.length && <p className="admin-muted">Aucune commande pour le moment.</p>}</div><div className="admin-panel"><div className="panel-heading"><div><p className="eyebrow">VIGILANCE</p><h3>Stock à surveiller</h3></div><span>◫</span></div>{lowStock.map((item) => <div className="admin-list-row" key={item.id}><div><strong>{item.name || item.sku}</strong><small>{item.sku || 'Produit'}</small></div><span className="status status-warning">{item.available} disponible{item.available > 1 ? 's' : ''}</span></div>)}{!lowStock.length && <p className="admin-muted">Aucune alerte de stock.</p>}</div></div></div>;
}
...
function AdminFormModal({ tab, item, token, onClose, onSaved, notify }) {
    ...
    const currentImage = tab === 'categories'
        ? item?.image_url
        : item?.images?.find((image) => image.is_primary)?.url || item?.images?.[0]?.url;
    ...
    const submit = async (event) => {
        ...
        try {
            ...
            if (tab === 'products' && !editing) {
                const productPayload = new FormData();
                Object.entries(requestPayload).forEach(([key, value]) => {
                    if (value !== null && value !== undefined) productPayload.append(key, typeof value === 'boolean' ? (value ? '1' : '0') : value);
                });
    ...
    return <div className="overlay admin-modal-overlay" onMouseDown={(event) => { if (!saving && event.target === event.currentTarget) onClose(); }}><div className="admin-form-modal" role="dialog" aria-modal="true" aria-labelledby="admin-form-title"><div className="drawer-head"><div><p className="eyebrow">{editing ? 'MODIFICATION' : 'NOUVEL ELEMENT'}</p><h2 id="admin-form-title">{title}</h2></div><button className="close-button" onClick={onClose} disabled={saving} aria-label="Fermer">×</button></div><form className="admin-form" onSubmit={submit}>{tab === 'categories' && <><label>Nom<input required value={values.name} onChange={update('name')} /></label><label>Description<textarea rows="3" value={values.description} onChange={update('description')} /></label><label className="full image-upload-field">Image de la catégorie<input required={!editing} type="file" accept="image/jpeg,image/png,image/webp" onChange={handleImageChange} />{imagePreview ? <span className="image-preview"><img src={imagePreview} alt="Aperçu de la catégorie" /></span> : <span className="image-preview-empty">Sélectionnez obligatoirement une photo pour la catégorie</span>}{imageFile && <small className="image-file-name">{imageFile.name}</small>}</label></>}{tab === 'products' && <><label>Nom<input required value={values.name} onChange={update('name')} /></label><label>Categorie<select value={values.category_id} onChange={update('category_id')}><option value="">Sans categorie</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>Prix (FCFA)<input required min="0" step="1" type="number" value={values.price} onChange={update('price')} /></label>{!editing && <><label>Quantite initiale<input required min="0" step="1" type="number" value={values.initial_stock} onChange={update('initial_stock')} /></label><label>Seuil d'alerte<input required min="0" step="1" type="number" value={values.reorder_level} onChange={update('reorder_level')} /></label></>}<label className="full image-upload-field">Image du produit<input required={!editing} type="file" accept="image/jpeg,image/png,image/webp" onChange={handleImageChange} />{imagePreview ? <span className="image-preview"><img src={imagePreview} alt="Aperçu du produit" /></span> : <span className="image-preview-empty">Sélectionnez obligatoirement une photo pour le produit</span>}{imageFile && <small className="image-file-name">{imageFile.name}</small>}</label><label className="full">Description<textarea rows="3" value={values.description} onChange={update('description')} /></label></>}{tab === 'suppliers' && <><label>Nom<input required value={values.name} onChange={update('name')} /></label><label>Contact<input value={values.contact_name} onChange={update('contact_name')} /></label><label>Email<input type="email" value={values.email} onChange={update('email')} /></label><label>Telephone<input value={values.phone} onChange={update('phone')} /></label><label className="full">Adresse<textarea rows="2" value={values.address} onChange={update('address')} /></label><label className="full">Notes<textarea rows="2" value={values.notes} onChange={update('notes')} /></label></>}<label className="admin-checkbox"><input type="checkbox" checked={values.is_active} onChange={update('is_active')} /> Element actif</label>{error && <p className="form-error">{error}</p>}<div className="admin-form-actions"><button type="button" className="button button-light" onClick={onClose} disabled={saving}>Annuler</button><button type="submit" disabled={saving} aria-busy={saving} className="button button-dark">{saving ? <><Spinner /> Enregistrement…</> : <>{editing ? 'Enregistrer les modifications' : 'Enregistrer'} <span>↗</span></>}</button></div></form></div></div>;
}
